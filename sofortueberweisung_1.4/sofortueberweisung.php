...
class Sofortueberweisung extends PaymentModule
{
    ...
    private function _displayForm()
    {
        ...
        // Assign filled array
        $smarty->assign($smartyAssignArray);
    ...
    public function execPayment($cart)
    {
        ...
        $language = $langObj->getLanguage($cart->id_lang);
        ...
        // Add smarty template tpl-frontend-redirect to the output
        $html = $this->display(__FILE__, 'tpl-frontend-redirect.tpl');
     ...
     * @return boolean
     */
    public function checkResponse($data, $log = null)
    {
        // Get hash fields
        ...
        if ($log) {
            ...
            $log->log('Incoming hash:');
            $log->log($data['hash']);

            $log->log('Internal data array:');
            $log->log($hashArray);
            
            $log->log('Imploded data:');
            $log->log($implodedHash);
            
            $log->log('Imploded hash:');
            $log->log($hash);
        }
    ...
    public function getPathSsl()
    {
        ...
        // Build SSL path
        $path .= htmlspecialchars($_SERVER['HTTP_HOST'], ENT_COMPAT, 'UTF-8') . __PS_BASE_URI__ . 'modules/' . $this->getName() . '/';
     ...
     * @return null
     */
    public function updateOrder($cartId, $state, $amount, $message)
    {
        $this->context->link = new Link();
        
        $this->validateOrder($cartId, $state, $amount, $this->getDisplayName(), $message);        
        return null;
    }
